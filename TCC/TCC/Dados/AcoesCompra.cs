...
namespace TCC.Dados
{
    public class AcoesCompra
    {
        ...
        public void inserirCompra(ModelCompra cm)
        {
            ...
            cm.dtCompra = Convert.ToString(date);
            MySqlCommand cmd = new MySqlCommand("insert into tbl_compra values(default,@codPag,@codCli,@valor,@qt,@dt_compra)", con.MyConectarBD());
            ...
            cmd.Parameters.Add("@qt", MySqlDbType.VarChar).Value = cm.qt_venda;
            cmd.Parameters.Add("@dt_compra", MySqlDbType.VarChar).Value = cm.dtCompra;
            cmd.ExecuteNonQuery();
            con.MyDesconectarBD();
        }
